...
import { RATE_PRESETS, UMA_PRESETS, ROUND_RESULTS } from '@/constants/mahjong';
...
export default function EditGameScreen() {
  ...
  const [gamePlayedAt, setGamePlayedAt] = useState('');
  ...
  async function loadData() {
    ...
    // 半荘情報をフォームにセット
    setGamePlayedAt(game.playedAt);
    setGameSessionId(game.sessionId);
    setRate(game.rate);
    // プリセットに該当しなければカスタム扱い
    const isPreset = RATE_PRESETS.some((p) => p.value === game.rate);
    if (!isPreset) {
    ...
    setUmaIndex(umaIdx >= 0 ? umaIdx : 0);

    // セッション配下の場合、セッション設定を読み込む
    if (game.sessionId) {
      const s = await sessionRepo.getById(game.sessionId);
      setSession(s);
    }

    // 局データを読み込み
    const existingRounds = await roundRepo.getByGameId(gameId);
    setRounds(
  ...
  // 収支計算に使う設定値（セッション配下はセッション設定を使用）
  const effectiveRate = isSessionGame ? session.rate : rate;
  const effectiveGameFee = isSessionGame ? session.gameFee : gameFee;
  const effectiveChipPrice = isSessionGame ? session.chipPrice : chipPrice;
  const effectiveUmaBig = isSessionGame ? session.umaBig : uma.big;
  ...
  async function save() {
    ...
    try {
      ...
      // 局データを全置換
      await roundRepo.replaceAll(gameId, rounds);
        ...
        <View style={styles.section}>
          ...
          {totalChipDelta !== 0 && (
            <View style={styles.chipSummary}>
              ...
              >
                {totalChipDelta >= 0 ? '+' : ''}
                {totalChipDelta}枚（¥{formatIncome(totalChipDelta * chipPrice)}）
              </Text>
            </View>
          )}
        </View>
